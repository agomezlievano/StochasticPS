...
gdp = gdp.merge(natural_resources, on = "exporter").rename(columns = {0: "nr"})

complexities = pd.DataFrame()
for year in range(1962, 2016):
   complexity = pd.read_csv("complexities_cp/%s.csv" % year, sep = "\t")
   complexity["year"] = year
   complexities = pd.concat([complexities, complexity])

table = complexities.merge(gdp.dropna(), on = ("exporter", "year"))
table.to_csv("eci_growth_regression_table_cp.csv", sep = "\t", index = False)
